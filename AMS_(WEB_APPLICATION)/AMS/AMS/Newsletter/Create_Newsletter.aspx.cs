...
public partial class Newsletter_Create_Newsletter : System.Web.UI.Page
{
    ...
    {
        if (IsPostBack && ddl_program.SelectedIndex.ToString() != "0")
        {
    ...
    protected void btn_send_Click(object sender, EventArgs e)
    {
        if (IsPostBack)
        {
            ...
            else if (accordion.SelectedIndex == 1)
            {
                ...
                ds1 = business_function.create_newsletter_detail(txt_subject.Text, txt_body.Text);
                string[] gr_no = txt_to.Text.ToString().Split(',');
                foreach (string a in gr_no)
                {
                    if (Regex.Match(a, @"\d+").Value.Length == 11 && ds1.Tables.Count > 0)
                    {
                        DataSet ds = new DataSet();
                        ds = business_function.create_newsletter_individual(Regex.Match(a, @"\d+").Value.ToString());
                        if (ds.Tables.Count > 0)
                        {
                            bool status = email_function.send_email(txt_body.Text, txt_subject.Text, ds.Tables[0].Rows[0][0].ToString());
                            if (status == true)
                            {
                                DataSet ds_success = new DataSet();
                                ds_success = business_function.create_newsletter_success(ds.Tables[0].Rows[0][5].ToString(), ds1.Tables[0].Rows[0][0].ToString());
